...
export default function CardList() {
  ...
  const [fastestElement, setFastestElement] = useState(null);

  async function fetchChecks() {
    try {
      const responses = await Promise.all(
        JSON.parse(process.env.NEXT_PUBLIC_SITES_TO_CHECK).map(async (site) => {
          ...
          const responseTimes = await axios.get(
            `${process.env.NEXT_PUBLIC_UPDOWN_BASE_URL}/api/checks/${site}/metrics?api-key=${process.env.NEXT_PUBLIC_UPDOWN_API_KEY}`
          );

          return { ...response.data, ...responseTimes.data };
        })
      );
      ...
      const maxElement = responses.reduce((prev, current) =>
        prev.timings.total > current.timings.total ? prev : current
      );

      setFastestElement(maxElement);
    } catch (error) {
      console.error(error);
    }
  }
  ...
  return (
    <Box mx="auto" borderBlockStart="1px solid var(--chakra-colors-gray-200)">
      {checksData?.length > 0 &&
        checksData.map((data) => {
          const { token, url, alias, last_check_at, uptime, timings } = data;
          return (
            <Box
              key={`${token}-${url}`}
